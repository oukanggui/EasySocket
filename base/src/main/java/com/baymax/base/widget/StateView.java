...
/**
 * Created by oukanggui on 2018/1/28.
 * 加载动画、加载出错、加载无数据
 */
public class StateView extends View {
    ...
    public View showRetry() {
        ...
        if (mRetryResource == R.layout.base_retry) {//使用默认的加载出错布局时，添加更具网络是否可用显示errortext，同时给在刷一刷按钮添加点击事件
            try {
                TextView errorTv = (TextView) mRetryView.findViewById(R.id.error_tv);
                if (!NetworkUtil.isNetworkAvailable(getContext())) {
                    errorTv.setText(getContext().getResources().getString(R.string.text_network_retry));
                } else {
                    errorTv.setText(getContext().getResources().getString(R.string.text_retry));
                }
                View refreshBtn = mRetryView.findViewById(R.id.error_refresh_tv);
                ...
                Log.e("StateView", "retryView inflat or bindView error!!");
            }
        } else {
